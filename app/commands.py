...
async def start_command():
    ...
    await SendMessage(context.user.user_id, T('start')).send()
...
async def upload_photo():
    ...
    resp = await SendMessage(context.user.user_id, mt).send()
    ...
    await EditMessageText(mt, context.user.user_id, resp.message_id, reply_markup=kb.render()).send()
...
async def upload_photo():
    ...
    if isinstance(link, dict):
        await SendMessage(context.user.user_id, T('errors/upload')).send()
        return
